...
@section('content')
    <div>
        ...
        <form action="{{ route('administrador.unitprograms.update', [$programa, $unidadPrograma]) }}" method="POST">
            @csrf
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="unit" id="unit" class="form-control">
                            ...
                            @foreach ($unidades as $unidad)
                                <option value="{{ $unidad->unid_id }}" @if ($unidad->unid_id == $unidadPrograma->unid_id) selected @endif>
            ...
            <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="relationType" id="relationType" class="form-control">
                            ...
                            @foreach ($relationTypes as $key => $relation)
                                <option value="{{ $key }}" @if ($key == $unidadPrograma->unpr_relacion) selected @endif>
                                    {{ $relation }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col text-center">
                    ...
                    @if ($unidadPrograma->unpr_esfacultad == 1)
                        <input name="isFaculty" type="checkbox" id="isFaculty" value="1" checked>
                    @else
                        ...
                        <select name="coveringType" id="coveringType" class="form-control">
                            ...
                            @foreach ($tipoCubrimientos as $tipoCubrimiento)
                                <option value="{{ $tipoCubrimiento->tcsn_id }}"
                                    @if ($unidadPrograma->cubrimiento->first() != null) @if ($tipoCubrimiento->tcsn_id == $unidadPrograma->cubrimiento->first()->tcsn_id) selected @endif
                                    @endIf>
                        ...
                        <select name="methodology" id="methodology" class="form-control">
                            ...
                            @foreach ($metodologias as $metodologia)
                                <option value="{{ $metodologia->meto_id }}"
                                    @if ($unidadPrograma->cubrimiento->first() != null) @if ($metodologia->meto_id == $unidadPrograma->cubrimiento->first()->pivot->meto_id) selected @endif
                                    @endIf>
